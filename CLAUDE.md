...
- `src/app/components/LockOpenIcon.tsx` — das GEÖFFNETE Schloss. **Nie `LockOpen` aus lucide nehmen** (`Unlock` ist nur ein anderer Name dafür): es unterscheidet sich vom geschlossenen `Lock` allein durch einen fehlenden Stummel von 5 px im 24er-Raster und ist bei den hier üblichen 11–18 px nicht als „offen" lesbar. Diese Fassung versetzt den Bügel, sodass sich die UMRISSLINIE unterscheidet, und ist über `createLucideIcon` gebaut — also ein echtes `LucideIcon` und in Tabellen dieses Typs einsetzbar
...
- `src/lib/theme.ts` — **welche Farbwelt gilt, und zwar abgeleitet statt gewählt.** `subWorld(isLocked)` → `sub-locked` (grün) oder `sub-open` (rosa), `keyholderWorld()` → `keyholder` (indigo), `DEFAULT_WORLD` für Bildschirme ohne Zustand (Anmeldung, `/info`). Alle drei sind DUNKEL; einen hellen Modus und einen Umschalter gibt es seit v6 nicht mehr. Gesetzt wird beim Rendern im Bereichs-Layout, `ThemeRootSync` trägt die Welt an `<html>` nach (für alles, was per Portal am Body hängt). **Eine neue Welt braucht drei Dinge:** einen Eintrag in `WELTEN` (`docs/design/tokens.mjs`), einen in `World`/`WORLDS` — `theme.test.ts` hält die beiden gegeneinander — und einen Lauf von `node docs/design/tokens.mjs --write`
...
- `src/lib/messageInboxRoutes.ts` — `makeInboxRoutes(resolveScope)`: die fünf Endpunkte EINES Posteingangs (`list`, `remove`, `markRead`, `markUnread`, `readAll`, `bulk`) als ein Bauplan. Daraus `ownInboxRoutes` (`/api/messages/*`, Scope aus der Session) und `keyholderInboxRoutes` (`/api/admin/messages/*`, Scope aus `requireControllerApi()`); die Route-Dateien sind nur noch `export const GET = …`. Eine neue Sicht auf den Posteingang bekommt einen Scope-Resolver, keine fünf neuen Dateien
