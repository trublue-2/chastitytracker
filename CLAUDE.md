...
- `src/app/components/OffenseStatementField.tsx` — die Stellungnahme des Trägers, an der Vergehens-Karte und an der Posteingangs-Zeile. **Sie entscheidet SELBST, ob sie etwas zeigt** — ein `&&` davor im Elter hängt sie aus und nimmt den halb getippten Text mit, weil ihre Server-Sicht mitten im Tippen wegfallen kann (`statementView`). Nur der Posteingang darf filtern, und das aus einem anderen Grund (dort klappt das Panel ohnehin zu). Dazu `OffenseCard`/`OffenseList` — dieselbe Karte im Sub-Dashboard und in der Keyholder-Sub-Übersicht
- `src/app/components/PenaltyActions.tsx` — die beiden Strafen-Aktionen an der Vergehens-Karte: `PenaltyReportButton` (Träger meldet „erledigt") und `PenaltyDoneButton` (Keyholderin schliesst ab), beide auf `useActionPatch`
...
- `src/lib/messageDefaults.ts` — `withMessageDefaults()`: Vorgaben für Platzhalter, die eine Meldung erst später bekommen hat (heute `proofCount` der Aufgaben-Meldungen). Angewandt in der Posteingangs-Anzeige UND beim Versand. **Ein neuer Platzhalter in einem bestehenden Meldungstext bekommt hier seine Vorgabe** — ältere Posteingangs-Zeilen tragen ihn nicht, und ohne Wert bricht das Rendern; eine zweite Fassung desselben Textes mit und ohne Zusatz ist die schlechtere Lösung
...
- `src/lib/utils.ts` — `buildWearPairs()`, `wearingHoursFromPairs()`, `isTimeCorrected()`, `formatDuration()`, `formatDateTime()`, `toDatetimeLocal()`, `tzOffsetMsAt()` (TZ-Offset-Mess-Primitiv, gecachte Formatter), `decomposeMs()` (ms → Tage/Std/Min/Sek), `groupByUser()` (Zeilen je `userId` bündeln) — **nie** wieder `Intl…formatToParts` für Offsets oder `% 86_400_000` von Hand
...
- `src/lib/mcp/stateAreas.ts` + `stateToken.ts` — `check_updates`: welche Tabelle zu welchem Zustands-Bereich eines Subs gehört. Daraus erzeugte SQLite-Trigger zählen JEDEN Insert/Update/Delete in `StateVersion` hoch (auch Poller und Portal-Roh-SQL). **Ein neues Modell mit `userId` gehört dort eingetragen** (zugeordnet oder mit Grund ignoriert), sonst meldet `check_updates` „nichts geändert", obwohl sich etwas geändert hat; `stateAreas.test.ts` bricht dann. Ändert sich das Register oder bekommt `User`/`BoxStatus` eine Spalte, braucht es eine neue Migration mit `npm run state-triggers`
