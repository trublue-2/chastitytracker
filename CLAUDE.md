...
- `src/lib/taskService.ts` — `checkTask()`/`mergeTaskPatch()` sind PRÜFEND und schreibfrei, getrennt von `writeTask()`. Genau deshalb ruft die MCP-dryRun-Vorschau sie auf (`mcpCreateTask`, `mcpEditTask`), statt ihre Schranken abzuschreiben — eine zweite Nachrechnung in der Vorschau läuft irgendwann auseinander und verspricht Erfolg für einen Commit, der mit 400 endet. **Eine neue Schranke gehört in `checkTask`, nicht in die Vorschau**; von dort erbt sie diese mit
