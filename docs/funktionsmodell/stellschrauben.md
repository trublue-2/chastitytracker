...
| `User.autoKontrolleFensterVon` | String | `""` | dauerhaft | Beginn eines optionalen festen Auslöse-Fensters. Leer = ganzes Wach-Fenster. Wrappt bewusst nicht über Mitternacht. | Keyholder (UI), Keyholder (MCP) | Auto-Kontrollen | `autoKontrolleDayRules.ts:fixedWindowMinutes` |
| `User.autoKontrolleFensterBis` | String | `""` | dauerhaft | Ende desselben Fensters. Liegt es vollständig im Schlaf-Fenster, wird die Kombination abgelehnt statt wirkungslos gespeichert. | Keyholder (UI), Keyholder (MCP) | Auto-Kontrollen | `autoKontrolleDayRules.ts:triggerWindowAllQuiet` |
...
| `User.postLockInspectionEnabled` | Boolean | `false` | dauerhaft | Nach JEDEM erfassten Verschluss folgt eine Kontrolle — zusätzlich zum Tagesplan. Eigenständig: weder der Hauptschalter der Automatik noch „nur bei Sperrzeit" gelten. Eingeschaltet übernimmt sie auch den Wiederverschluss nach einer Reinigungspause. | Keyholder (UI), Keyholder (MCP) | Auto-Kontrollen, Kontrollen, Strafbuch | `autoKontrolleService.ts:schedulePostLockInspection` |
