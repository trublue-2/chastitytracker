...
import {
  withdrawVerschlussAnforderung, withdrawVerschlussAnforderungById, updateSperrzeitEnde,
...
describe("updateSperrzeitEnde", () => {
  ...
  it("BUG 14.07.: eine GEPLANTE Sperrzeit zu ändern schickt KEINE Mail", async () => {
    ...
    const res = await updateSperrzeitEnde("s1", ZUKUNFT);
    ...
    expect(updateMock).toHaveBeenCalledWith({ where: { id: "s1" }, data: { endetAt: ZUKUNFT } });
  });
  ...
  it("die Guards greifen vor allem anderen — kein Schreiben, keine Meldung", async () => {
    ...
    expect((await updateSperrzeitEnde("s1", ZUKUNFT)).ok).toBe(false);
