...
describe("mcpImageKeyUnlocked — an DIESE Instanz gebunden", () => {
  ...
  it("bleibt aus ohne Schlüssel, ohne Benutzer und bei einem Datenbank-Fehler", async () => {
    db.user.findUnique.mockResolvedValue({ id: "u1" });
    vi.stubEnv("MCP_USERNAME", "sub");
    vi.stubEnv("MCP_IMAGE_KEY", undefined);
    expect(await mcpImageKeyUnlocked()).toBe(false);
    vi.stubEnv("MCP_USERNAME", undefined);
    vi.stubEnv("MCP_IMAGE_KEY", "u1");
    expect(await mcpImageKeyUnlocked()).toBe(false);
  ...
  it("ein unbekannter MCP_USERNAME schaltet nicht frei", async () => {
    db.user.findUnique.mockResolvedValue(null);
    vi.stubEnv("MCP_USERNAME", "gibtsnicht");
    vi.stubEnv("MCP_IMAGE_KEY", "u1");
    expect(await mcpImageKeyUnlocked()).toBe(false);
  });
