...
describe("keyholderDashboard — V2-Feldbestand", () => {
  it("liefert exakt die Vertragsfelder", async () => {
    ...
    expect(result.schemaVersion).toBe(18); // v18: minDurationHours→minDurationHours, beginsAt→beginsAt (Eingang und Ausgang gleich); v17: `endetAt` heisst `endsAt` (Englisch an der MCP-Oberfläche); v16: auch goal*H ist null, wo eine Zielgrenze in der Periode liegt; v15 trug zwei Umdeutungen desselben Tages: goals folgt period_summary v3 — Prozentwerte null, wo eine Zielgrenze in der Periode liegt (goalChangedInPeriod), Tagesziel am Anbruchtag null; und `weight` folgt dem Zielgewicht — corridor/breach weg, target/remainingKg/reached an ihrer Stelle; v14: die Orgasmus-Anweisung ist terminierbar — openOrgasmWindow zeigt nur noch GELTENDE Fenster, geplante stehen in scheduledDirectives (kind: orgasm); v13: späte Annahme rettet — der Zustand einer Aufgabe ist nach einer verstrichenen Nachweis-Frist nicht mehr endgültig; v12: Nachweise mit eigener Fälligkeit — je Nachweis `dueAt`; eine Aufgabe kann seither MITTEN in ihrer Laufzeit versäumt sein und aus openTasks fallen (der damals mit angekündigte Nachweis-Zustand `overdue` kommt dort nie vor, korrigiert 16.08.2026); v11: openTasks zeigt nur noch AUSGELÖSTE Aufgaben — terminierte stehen in scheduledDirectives; v10: openTasks.holdUntil ist das WIRKSAME Ende (Dauer-Modus: startedAt + holdDurationMin); v9: openControl → openControls (je Ziel eine Kontrolle); v8: openTasks enthält auch `awaitingReview` (+ awaitingYourReview); v7: openControl.code nullable; v6: openLockRequest = die DRINGENDSTE von mehreren
