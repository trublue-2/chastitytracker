...
import {
  mcpImageMaxPx, MCP_IMAGE_MAX_AGE_H, MCP_IMAGE_MAX_AGE_MS, MCP_IMAGE_PER_HOUR, MCP_IMAGE_PER_DAY,
} from "@/lib/constants";
 ...
 *
 * Der Unterschied ist eine Betriebs-Eigenschaft, keine Sicherheits-Lücke: Diese Antwort wird EINMAL
 * pro Prozess ausgewertet und dann festgehalten — `buildAuthHandler` fragt sie beim Bau des Handlers,
 * der über `authHandlerPromise` memoisiert ist, und hält das Ergebnis in seiner Closure. (Die
 * Werkzeug-Registrierung selbst läuft dagegen pro Request neu: `mcp-handler` baut je Anfrage einen
 * frischen `McpServer`. Wer hier teure Einmal-Arbeit einhängt, zahlt sie also jedes Mal.)
 * Trifft der erste MCP-Request ein, während die Datenbank noch nicht antwortet (Migration beim
 * Container-Start), fiele das Werkzeug bis zum nächsten Neustart aus der Liste — ohne Selbstheilung
 * und für den Betreiber nur an einer Logzeile erkennbar. Ein Ausfall ist aber kein Nein.
 *
...
async function deliver(userId: string, imageUrl: string, caption: string, recordedAt: Date | null): Promise<McpImage> {
  ...
  if (!recordedAt || Date.now() - recordedAt.getTime() > MCP_IMAGE_MAX_AGE_MS) {
    throw new Error(`${caption} — older than ${MCP_IMAGE_MAX_AGE_H}h. Images are only available while the entry is fresh; this is a rule, not a fault. Ask the user directly if you need to see it.`);
  }
