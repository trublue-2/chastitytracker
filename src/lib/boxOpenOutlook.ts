...
export interface BoxHoldParams {
  ...
  lockPeriod: { endsAt: string | null; indefinite: boolean } | null;
...
export function boxHoldOutlook(p: BoxHoldParams): BoxHold | null {
  ...
  // Eine UNBEFRISTETE Sperrzeit hat kein `endsAt`. Nur aus dem fehlenden Datum auf „öffnet" zu
  ...
  const running = !!p.lockPeriod && (p.lockPeriod.indefinite || (!!p.lockPeriod.endsAt && new Date(p.lockPeriod.endsAt) > p.now));
