...
const cleaning = { allowed: false, maxMinutes: 15 };
...
describe("deviceConfidence — undeclared vs. declared (A-04)", () => {
  it("VERSCHLUSS ohne Gerät und ohne Kontrolle → undeclared, nicht declared", () => {
    const sessions = buildSessions([entry("VERSCHLUSS", "2026-07-17T00:00:00Z", null)], cleaning, NOW);
  ...
  it("VERSCHLUSS MIT Gerät und ohne Kontrolle bleibt declared (Regression)", () => {
    const sessions = buildSessions([entry("VERSCHLUSS", "2026-07-17T00:00:00Z", { id: "d1", name: "Flatty", categoryId: null })], cleaning, NOW);
    const seg = sessions[0].segments[0];
