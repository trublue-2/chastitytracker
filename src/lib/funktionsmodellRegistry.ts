...
export const FM_REGISTRY: FmEntry[] = [
  ...
  s({
    model: "Entry", field: "openAwaitsBolt", domain: "eintraege", scope: "entry",
    effect: "Nur LockMeBox: die Öffnung ist erst der Aufruf, die Box zu öffnen. `true` = für JEDE Ableitung unsichtbar, der Träger bleibt verschlossen, bis die Box „Riegel offen\" meldet; dann wird sie vollzogen und das Feld gelöscht.",
    writers: ["system"], affects: ["Box", "Sperrzeit", "Sessions/Statistik", "Strafbuch"],
    anchor: "lockPending.ts",
  }),
  s({
  ...
  x("runtime", "Task", "proofReminderSentAt",
    "Versand-Stempel der Erinnerung kurz vor der Nachweis-Frist — einmal je Aufgabe."),
  ...
  x("runtime", "StrafeRecord", "reportedDoneAt",
    "Der Träger meldet die Strafe als erledigt. Schliesst nichts — `erledigtAt` setzt weiterhin nur die Keyholderin; neues Urteil und „Wieder offen“ leeren es."),
  stamp("StrafeRecord"),
  ...
  x("record", "BoxStatus", "name", "Anzeigename der Box; kommt aus Heimdall bzw. ist bei der LockMeBox ihr Bluetooth-Name."),
  x("identity", "BoxStatus", "kind",
    "Welche Box: `heimdall` meldet sich selbst, `lockmebox` (Werks-Firmware) nur, wenn sich das Handy des Trägers per Bluetooth verbindet."),
  x("record", "BoxStatus", "lockPassword",
    "Nur LockMeBox: das Passwort, mit dem der Tracker sie verschliesst und öffnet. Einmal je Box beim Koppeln erzeugt, nie gewechselt, verlässt nie den Server."),
