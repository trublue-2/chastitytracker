...
export default function DeviceFormSheet({ open, onClose, onSaved, device, userId }: Props) {
  ...
  useEffect(() => {
    if (open) {
      ...
      setPrice(device?.purchasePrice != null ? String(device.purchasePrice) : "");
      setCurrency(device?.currency ?? "CHF");
      setError("");
    }
  }, [open, device]);
  ...
  return (
    <Sheet open={open} onClose={onClose} title={device ? t("editDevice") : t("addDevice")}>
