...
export async function POST(req: NextRequest) {
  try {
    const { userId, art, nachricht, beginntAt, endsAt, vorgegebeneArt, oeffnenErlaubt, delayMinutes, wirksamAbAt } = await req.json();

    const actor = await requireKeyholderOrAdminActor(userId);
    if (actor instanceof NextResponse) return actor;

    const result = await createOrgasmusAnforderung(
      { userId, art, nachricht, beginntAt, endsAt, vorgegebeneArt, oeffnenErlaubt, delayMinutes, wirksamAbAt },
      sessionActor(actor),
    );
