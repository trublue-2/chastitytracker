...
export async function POST(req: NextRequest) {
  try {
    const { userId, art, message, beginsAt, endsAt, requiredType, openingAllowed, delayMinutes, wirksamAbAt } = await req.json();

    const actor = await requireKeyholderOrAdminActor(userId);
    if (actor instanceof NextResponse) return actor;

    const result = await createOrgasmusAnforderung(
      { userId, art, message, beginsAt, endsAt, requiredType, openingAllowed, delayMinutes, wirksamAbAt },
      sessionActor(actor),
    );
