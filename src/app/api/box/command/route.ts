...
export async function POST(req: NextRequest) {
  ...
  if (box.locked) return NextResponse.json({ error: "Box ist bereits verschlossen" }, { status: 400 });

  await prisma.boxStatus.update({
    where: { userId_boxId: { userId, boxId } },
    data: { pendingCommand: "lock", pendingCommandAt: new Date(), pendingCommandRelockBy: null },
  });
