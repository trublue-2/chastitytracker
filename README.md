...
- **Cleaning openings** with configurable daily limits, per-opening max-minutes, and optional time windows **per weekday**. Once any window exists, a weekday none of them covers is a *closed* day — cleaning is forbidden all day, which is how you rule out a single day. Changing any of these applies **from that moment on**: the penalty log keeps judging a past opening by the rules that were in force when it happened
...
- **Inbox** — every notification a user receives (inspection, lock & closure, orgasm, penalty, system) also lands in an in-app inbox, colour-coded by category, so nothing is lost when an email doesn't arrive. Read state is per message (opening the list is not enough), messages can be marked unread again, acknowledged all at once, or deleted; an open inspection links straight to the inspection form with the code pre-filled. The bell and the app icon carry the unread count. Notifications go out by mail, push and Telegram (a bot-based third channel, enabled per instance). Each channel carries a level the recipient sets for themselves — everything, important only, or off — and it applies to every notification reaching that person, including the ones about their wearers. How urgent a notification is comes from the notification itself, not from a per-event switch. Anything with a deadline still gets through: if none of the switched-on channels can reach the user, it goes to every channel that can (details: [docs/nachrichten-konzept.md](docs/nachrichten-konzept.md)). A user who blocks the Telegram bot is disconnected automatically, and the settings warn when no switched-on channel reaches them. The settings hold both halves of each channel in one place and in the same order — first whether it has a target at all (mail address, Telegram chat, push on this device), then how loud it may be. A channel that cannot deliver shows its level greyed out and says why, instead of promising something it cannot keep. Keyholders have an inbox of their own behind the same bell: notices about their wearers (auto-filed inspections, task results, late proofs)
...
- **His side of the penalty log** — the wearer sees the offenses recorded against him and can state his case on each one, until the judgment is passed. What he writes stands above the keyholder's verdict field, so she reads it before deciding; afterwards it stays as a quote. If the offense is punished while he is still writing, his text is not swallowed — the card stays and he is told that the offense has since been judged. Statements are switchable per wearer. Once a penalty is imposed, he reports it done; closing it stays with the keyholder
...
- **Automatic inspections** — randomly scheduled across the day within a configurable wake window. Sleep window and the optional fixed trigger window can differ **per weekday**, and individual weekdays can be left out of the plan entirely. The inspection that follows a re-lock after a cleaning break is not affected by those rest days — it answers an action of the wearer, not a schedule. Concept: `docs/tagesfenster-konzept.md`
...
- **Penalty tracking** — fourteen kinds of offense, all but the last derived live from the entries: unauthorized opening, inspection late / rejected / unanswered, cleaning limit exceeded, no re-lock after cleaning, wrong device, missed lock request, missed orgasm instruction, unauthorized orgasm, unfulfilled task, missed weight report (only where weight tracking is enabled), admin-password change during a lock period, plus offenses recorded by hand
...
Published to `ghcr.io/trublue-2/chastitytracker` on three rolling channels, `:feature` → `:portal` → `:latest`:

| Tag | Purpose |
| --- | --- |
| `:latest` | Official release. Only moved by an explicit promotion — **use this one** |
| `:portal` | Every build from `main`. Not release-gated |
| `:feature` | Feature branches and pre-release builds |

Immutable tags are also published for the current build: `v<version>` (from `main` only) and `sha-<commit>`. Pin one of those if you need a fixed target — but note the removal below.
...
src/
  ...
  lib/
    ...
    notify.ts               # Notification dispatch (inbox entry + mail/push/Telegram per level)
    deliveryChannels.ts     # Which channels deliver: recipient levels, deadline fallback
...
| `POST` | `/api/offense-statement` | Wearer's own side: write, change or withdraw his statement on one offense. Refused once the offense has been judged (`409`) or where statements are switched off for him (`403`) |
| `PATCH` | `/api/penalty-done` | Wearer reports an imposed penalty as done (closing it stays with the keyholder) |
...
| `PATCH` | `/api/settings/notify-levels` | Set the level of one own channel: everything, important only, or off |
| `PATCH` | `/api/settings/notifications` | Change own mail / push / Telegram switch for the weigh-in reminder |
...
| `NotificationPreference` | The one remaining per-event switch: mail/push/Telegram for the weigh-in reminder. The general levels live on the account (`User.notifyMail` / `notifyPush` / `notifyTelegram`) |
